...
import { UpdateMeetDto, UpdateMeetObjectDto } from './dtos/updatemeet.dto';
...
export class MeetService {
    ...
    async createMeet(userId: string, dto: CreateMeetDto) {
        ...
        this.logger.debug('create - payload before save', meet);
    ...
    async getMeetObjects(meetId: string, userId: string) {
        this.logger.debug(`getMeetByUser - ${userId} - ${meetId}`);
        const user = await this.userService.getUserById(userId);
